...
class decay_group:
    ...
    def __init__(self,
            data, irf, channel=None, irf_channel=None, t_data=[], mask=None, fit_start_bin=None, fit_end_bin=None, modeltype="2exp", npx=1,
            # trunc=False, peak_start=1.5, peak_end=3,
            # bgsub=False, bg_start=7, bg_end=9
            trunc=False, peak_start=2.5, peak_end=4,
            # bgsub=False, bg_start=9, bg_end=11
            bgsub=False, bg_start=8, bg_end=10
        ):

        self.irf          = irf
        self.t_irf        = irf.times
        self.dt_irf       = self.t_irf[1] - self.t_irf[0]
        self.dc_irf       = irf.decay_curve(
                                channel=irf_channel,
                                plot=False, normalize=True,
                                trunc=True, peak_start=peak_start, peak_end=peak_end,
                                bgsub=True, bg_start=bg_start, bg_end=bg_end
                            )
    ...
    def load_data(self, data, channel=None, mask=None, fit_start_bin=None, fit_end_bin=None, npx=1):
        ...
        if isinstance(data, np.ndarray) and len(data.shape) == 1:
            ...
            # self.data         = data
            self.t_data       = data.times
            self.dt_data      = self.t_data[1] - self.t_data[0]
            self.dc_data      = data.decay_curve(channel, plot=False, normalize=False, mask=mask)
